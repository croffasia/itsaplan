...
import NoteImageNode from './NoteImageNode';
import NoteCanvasTitle from './NoteCanvasTitle';
import NoteCanvasControls from './NoteCanvasControls';
import { toFlowNodes, toFlowEdges, newSticker, type NoteFlowNode } from '../utils/noteCanvas';
...
import { useNoteImageUpload } from '../hooks/useNoteImageUpload';
import NoteCanvasImageProvider from '../context/NoteCanvasImageContext';
...
export default function NoteCanvas({
  ...
  const nodeTypes = useMemo(() => ({ sticker: StickerNode, image: NoteImageNode }), []);
  const [nodes, setNodes, onNodesChange] = useNodesState<NoteFlowNode>(toFlowNodes(board.canvas));
  ...
  const imageLimits = useStorageSettingsQuery().data;
  const imageUpload = useNoteImageUpload(projectKey, board.id, imageLimits);
  ...
  return (
    ...
    >
      ...
      <NoteCanvasControls
        ...
        onAddImages={imageUpload.upload}
      ...
      <NoteCanvasImageProvider projectKey={projectKey} boardId={board.id} canEdit={canEdit}>
        <ReactFlow
          nodes={nodes}
          edges={edges}
          nodeTypes={nodeTypes}
