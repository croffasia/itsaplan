...
import { ColumnSearchProvider } from '../search/ColumnSearchProvider';
...
export default function KanbanBoard(props: WorkItemsViewProps) {
  ...
  const searchScope = `${pathname}:${props.settings.group}:${props.settings.subgroup}`;
  ...
  return (
    <SelectionProvider validIds={validIds}>
      <ColumnSearchProvider key={searchScope} board={props} scope={searchScope}>
        {props.settings.subgroup === 'none' ? (
          <FlatBoard {...props} />
        ) : (
          <SwimlaneBoard {...props} />
        )}
