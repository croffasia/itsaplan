...
`WEBHOOK_CLEANUP_EVERY_TICKS`. `DATABASE_URL` and `WORKER_INTERNAL_TOKEN` are
required: `internal-api.ts` checks the token when it loads (`assertStrongSecret`
from `@repo/crypto`, so a short or example value stops the process at startup).
Agent runs and notification delivery additionally need an api origin
(`SERVICE_URL_API`, else `API_URL`); `postInternal` throws when it is missing, no
fallback origin.
